...
```{r}
#na.rm=TRUE we need because there are 4 values that are null values but this tells it to remove the nas
...
```
range(MediaSalary$Salary, na.rm=TRUE)
nrow(MediaSalary)
MediaSalary%>% 
  filter (Salary > 500)%>% 
  nrow ()
...
```{r}
#1: Build a table for NewYorkTimes employees   
NYT <- subset(MediaBucks, COMPANY%in% c("NewYorkTimes", "NYT", "Wirecutter_NYT")) 
summary(NYT$Salary)
#AI use disclosure here. When I used the COMPANY= command, it wasn't generating a summary. I asked chatGPT and it said I have to use the %in% command instead because the first command only takes one logical condition while this second command allows for the use of three separate filters. I looked at the excel spreadsheet and saw that some people identified as New York Times, and others as NYT or Wirecutter_NYT
...
#I used AI (chatGPT) because I wasn't sure how to find max value because the example code given showed ways to filter for values above a particular number. Now, I understand that you can just use max(Salary) to find that value. I kept NA values as true here because it doesn't matter if there are null values because I'm just looking for one row rather than trying to do any calculations, I think. 
```
...
#Which one is higher? ANSWER: Bloomberg reporters have a higher mean and median salary than Business Insider, although Business Insider has a higher maximum salary than Bloomberg.

Business <-subset(MediaSalary, COMPANY == "Business Insider")
summary(Business$Salary)
Bloomberg <-subset(MediaSalary, COMPANY == "Bloomberg")
...
MediaSalary%>%
  ...
  select (TITLE, COMPANY, Salary, Gender, Race)
  
##Question 2
  
## Line 379. Make table of editors. Many different types of editors in TITLE column. Went through manually to try and pull these different names out, but probably missed some title names. Is there a better way to do this? Update, kept getting errors so just switched to "Editor" for now. Need to get some help on this.
...
## Question 4
## AI disclosure (chatGPT) i looked up how to identify "data" in column even if other words are attached to it. chatGPT said the grepl() function will allow me to look for a pattern inside text.
Data <-MediaSalary %>%
